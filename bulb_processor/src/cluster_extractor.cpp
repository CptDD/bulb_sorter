...
void segment_clouds(vector<pair<string,pcl::PointCloud<pcl::PointXYZ>::Ptr> >clouds)
{
	...
	for(int i=0;i<clouds.size();i++)
	{
		...
		cout<<"Extracted clusters :"<<clusters.size()<<endl;

		string cloud_name=clouds[i].first;
		...
		for(int j=0;j<clusters.size();j++)
		{
			...
			if(j==0)
			{	
				ss<<cloud_name<<"_principal.pcd";
			}else
			{
				ss<<cloud_name<<"_secondary.pcd";
			}


			cout<<"Cluster name :"<<ss.str()<<endl;
			save_cloud(clusters[j],ss.str(),"livarno");
		}
	}
}
