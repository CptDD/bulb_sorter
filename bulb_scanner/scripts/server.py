...
def valid_points_to_file(points,filename):
	...
	for i in range(0,len(points)):
		...
		out.write('Original '+temp['label']+'\n')
		out.write('Position :'+str(temp['original'][0][0])+'\t'+str(temp['original'][0][1])+'\t'+str(temp['original'][0][2])+'\n')
		out.write('Orientation :'+str(temp['orientation'][0])+'\t'+str(temp['orientation'][1])+'\t'+str(temp['orientation'][2])+'\t'+str(temp['orientation'][3])+'\n')
		out.write('_______________\n')

		content='Neighbours ';
		for j in range(len(neigh_points)):
			label=neigh_labels[j].replace('p','')
			#out.write(label+' ',end='')
			content+='\t'		
			content+=label
			#out.write(str(neigh_points[j][0])+'\t'+str(neigh_points[j][1])+'\t'+str(neigh_points[j][2])+'\n')
...
def move_to_home(arm):
	...
	ns = "ExternalTools/" + arm_limb + "/PositionKinematicsNode/IKService"
...
def move_to_valid(arm,valid):
	...
	pose.pose.position.x=valid['original'][0][0]
	pose.pose.position.y=valid['original'][0][1]
	pose.pose.position.z=valid['original'][0][2]

	pose.pose.orientation.x=valid['orientation'][0]
	pose.pose.orientation.y=valid['orientation'][1]
	pose.pose.orientation.z=valid['orientation'][2]
	pose.pose.orientation.w=valid['orientation'][3]
...
def handle(req):
	...
	left = baxter_interface.Limb(arm_limb)
	...
	for point in graph:
		...
		if(resp.isValid[0]):
			...
			temp_point=dict()
			temp_point['original']=point['original']
	...
	for i in range(0,len(valid_points)):
		...
		move_to_valid(left,valid_points[i])


		for j in range(0,len(valid_points)):
			if i!=j:
				print('Trying to move to temp point :'+str(valid_points[j]['label']))

				if(move_to_valid(left,valid_points[j])):
