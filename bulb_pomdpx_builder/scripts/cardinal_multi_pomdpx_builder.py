...
def main():
	...
	points_filename=path+'/graph/valid.txt'
	path=rospack.get_path('bulb_processor')

	elongated_first_filename=path+'/results/multi/ee.txt'
	elongated_second_filename=path+'/results/multi/le.txt'

	livarno_first_filename=path+'/results/multi/ll.txt'
	livarno_second_filename=path+'/results/multi/el.txt'
	...
	if 'elongated' in bulbs:
		elong_first_percs=read_percentages(elongated_first_filename)
		elong_second_percs=read_percentages(elongated_second_filename)

	if 'livarno' in bulbs:
		livarno_first_percs=read_percentages(livarno_first_filename)
		livarno_second_percs=read_percentages(livarno_second_filename)


	percs=[]

	if 'elongated' in bulbs:
		percs.append(elong_first_percs)
		percs.append(elong_second_percs)
	if 'livarno' in bulbs:
		percs.append(livarno_first_percs)
		percs.append(livarno_second_percs)
